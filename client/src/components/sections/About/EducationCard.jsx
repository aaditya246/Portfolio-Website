  ...
  return (
    <GlassCard className="flex flex-col sm:flex-row sm:items-start gap-5">
      ...
      <div className="flex flex-col gap-2">
        <h3 className="font-display text-lg sm:text-xl font-semibold text-slate-900 dark:text-ink leading-snug">
          {item.institution}
        </h3>
        <p className="text-slate-900 dark:text-ink-muted text-sm sm:text-base">{item.degree}</p>

        <div className="flex flex-wrap items-center gap-3 mt-1">
          <span className="font-mono text-xs px-3 py-1 rounded-full border border-emerald-200 dark:border-border bg-panel-light/60 text-slate-900 dark:text-ink-faint">
            {item.duration}
          </span>
