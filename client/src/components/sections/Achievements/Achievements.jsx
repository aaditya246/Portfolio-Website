          ...
          {achievements.map((item) => (
            <motion.div key={item.id} variants={staggerItem}>
              <GlassCard className="h-full flex flex-col gap-3">
                ...
                <h3 className="font-display text-base sm:text-lg font-semibold text-slate-900 dark:text-ink">
                  {item.title}
                </h3>
                <p className="text-slate-900 dark:text-ink-muted text-sm leading-relaxed">{item.description}</p>
                <div className="flex items-center justify-between mt-auto pt-2">
                  <span className="font-mono text-xs text-slate-900 dark:text-ink-faint">{item.organization}</span>
